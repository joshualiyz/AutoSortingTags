...
@interface OrderingTagsView ()<TagViewTouchDelegate>
{
    CGFloat gap;
    CGFloat itemWidth;
...
@implementation OrderingTagsView
...
- (void) configView
{
    ...
    gap = 20;
    itemWidth = (self.frame.size.width - 50 - 20 * (COLUM_NUM - 1)) / COLUM_NUM;
...
- (void) tagView:(UIView *)view touchesBegan:(NSSet<UITouch *> *)touches
{
    ...
    for(NSInteger i = 0 ; i < self.tags.count ; i ++){
        ...
        if(CGRectContainsPoint(tag.view.frame, startDraggingOrigin)){
            ...
            [self addSubview:curDraggingTag.view];
            break;
        }
    }
}
